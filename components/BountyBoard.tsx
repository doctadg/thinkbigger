...
import { Lock, Rocket, Flame, Orbit, Trophy } from "lucide-react";
import { BOUNTIES } from "@/lib/constants";

const EASE = [0.22, 1, 0.36, 1] as const;

const ICONS = [Flame, Rocket, Trophy, Orbit, Trophy];
          ...
          {BOUNTIES.map((bounty, i) => {
            ...
            return (
              <motion.div
                ...
                className={`group relative grid grid-cols-12 gap-4 lg:gap-6 items-center p-5 lg:p-7 rounded-2xl border transition-all duration-500 ${
                  isActive
                    ? "border-[var(--color-gold)] bg-gradient-to-r from-[var(--color-gold-dim)] to-transparent"
              ...
              >
                ...
                <div className="col-span-3 lg:col-span-2 flex items-center gap-3">
                  <div
                    className={`w-11 h-11 lg:w-12 lg:h-12 rounded-xl flex items-center justify-center shrink-0 ${
                      isActive
                        ? "bg-[var(--color-gold)] text-black"
                        : "bg-[var(--color-surface-3)] text-[var(--color-muted)]"
                    }`}
                  >
                    {isActive ? <Icon size={20} /> : <Lock size={16} />}
                  </div>
                ...
                {isActive && (
                  <div className="absolute top-3 right-3 lg:top-5 lg:right-5">
                    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-[var(--color-gold)] text-black text-[10px] font-mono font-bold tracking-wider uppercase">
